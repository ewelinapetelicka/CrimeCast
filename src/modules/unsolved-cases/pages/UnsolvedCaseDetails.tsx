...
export function UnsolvedCaseDetails() {
    ...
    return (
        <PageContainer class={"text-neutral-50  flex flex-col justify-between"} scroll={true}>
            <h2 className={"text-2xl font-bold mb-2 bg-white bg-opacity-20 rounded-3xl shadow-inner shadow-neutral-50 cursor-pointer p-2 pl-3"}>
                {data.caseName}
            </h2>
            <section className={'pl-4'}>
                ...
                <div className={"mb-6"}>
                    ...
                    {data?.lastKnownActivity !== null ? (
                        <p>{data.lastKnownActivity}</p>
                    ) : (
                        <p>Unknown</p>
                    )}
                </div>
            ...
            <div className={"mb-4"}>
                <button
                    className={'w-2/12 mb-2 bg-white bg-opacity-20 rounded-3xl shadow-inner shadow-neutral-50 p-2 uppercase'}
                    onClick={() => navigate('/unsolved-cases')}>back
                </button>
            </div>
        </PageContainer>
    )

...
export function UnsolvedCaseForm(props: UnsolvedCaseFormProps) {
  ...
  const errorText = (
    <section className={'w-full h-52 bg-zinc-900 rounded-xl text-neutral-50 flex flex-col'}>
      <button onClick={() => setErrorWindow(false)} className={'self-end p-3'}>
        <IoIosClose />
      </button>
      ...
      {errorWindow ? (
        ...
        <section className={'flex flex-col justify-between h-full text-neutral-50'}>
          <article className={'flex w-full justify-between'}>
            ...
            <button className={button.text} onClick={() => props.onBackClicked()}>
              BACK TO CASES LIST
            </button>
          </article>
          <article className={'flex flex-col justify-evenly h-full'}>
            <div className={'flex flex-col gap-1 pb-3'}>
              ...
              <input
                ...
                onChange={(event) =>
                  setUnsolvedCase({ ...unsolvedCase, caseName: event.target.value })
                }
              />
            </div>
            <div className={'flex flex-col gap-1 pb-3'}>
              ...
              <textarea
                className={'w-full shadow-inner shadow-zinc-500 rounded bg-zinc-600'}
                rows={6}
                onChange={(event) =>
                  setUnsolvedCase({
                    ...unsolvedCase,
            ...
            <section className={'flex gap-6 pb-3 w-full justify-between'}>
              <div className={'flex flex-col gap-1 w-1/2'}>
                ...
                <input
                  ...
                  type={'date'}
                  onChange={(event) =>
                    setUnsolvedCase({ ...unsolvedCase, date: event.target.value })
                  }
                />
              </div>
              <div className={'flex flex-col gap-1 w-1/2'}>
            ...
            <section className={'flex gap-6 pb-3 w-full justify-between'}>
              <Chips
                value={unsolvedCase.keyEvidence}
                label={'key evidence'}
                onValueChange={(keyEvidence) =>
                  setUnsolvedCase({
                    ...unsolvedCase,
              ...
              ></Chips>
              <Chips
                value={unsolvedCase.suspects}
                label={'suspects'}
                onValueChange={(suspects) =>
                  setUnsolvedCase({ ...unsolvedCase, suspects: suspects })
                }
              ></Chips>
            </section>
            <button
              className={classNameJoin(button.text, 'flex self-end pr-1')}
